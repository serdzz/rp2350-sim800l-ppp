...
pub async fn prepare<A: AtatClient>(
    ...
    // 2a. Приём времени от сети (NITZ). Обязательно ДО AT&W ниже: настройка
    //     вступает в силу лишь после перезапуска модуля, поэтому её нужно
    //     сохранить в профиль, иначе она не заработает никогда.
    if let Err(e) = client.send(&SetNetworkTime { mode: 1 }).await {
        warn!("SIM800L: приём времени от сети не включён: {:?}", e);
    }

    // 2b. Закрепляем скорость, эхо и приём времени в профиле модуля: после
    //     сброса по питанию модуль иначе вернётся в автобод и начнёт отвечать
    //     на чужой скорости.
