...
J2 = {1: "+3V3", 2: "GND", 3: "GP3", 4: "GP4", 5: "GP5",
      6: "GP6", 7: "GP7", 8: "GP8", 9: "GP9", 10: "GND"}
...
SYMS = {
    ...
    "J1x10": ("Connector_Generic", "Conn_01x10"),
    "J1x2": ("Connector_Generic", "Conn_01x02"),
    "+3V3": ("power", "+3V3"),
    "+12V": ("power", "+12V"),
    "GND": ("power", "GND"),
    "FLAG": ("power", "PWR_FLAG"),
...
def pwr_flag(kind, x, y):
    ...
    Нужна ERC: питание приходит с гребёнки и с клеммника, то есть с обычных
    пассивных выводов, и без флага проверка считает шины ничем не запитанными.
    """
...
# Питание приёмника: 10–16 В, около 20 мА.
p = place("J1x2", "J3", "+12V IN", 24 * U, 40 * U,
          "TerminalBlock_Phoenix:TerminalBlock_Phoenix_MKDS-1,5-2-5.08_1x02_P5.08mm_Horizontal",
          prop=(-3.81, -8.89))
for n, net in ((1, "+12V"), (2, "GND")):
    x, y = p[str(n)]
    wire(x, y, x - 6.35, y)
    label(net, x - 6.35, y)

# Гребёнка к плате. 3.3 В берём отсюда же — отдельного источника плате не надо.
p = place("J1x10", "J2", "RP2350-Plus", 24 * U, 67 * U,
          "Connector_PinHeader_2.54mm:PinHeader_1x10_P2.54mm_Vertical", prop=(-3.81, -16.51))
for n, net in J2.items():
    x, y = p[str(n)]
    wire(x, y, x - 6.35, y)
    label(net, x - 6.35, y)

# Питание приходит с гребёнки и клеммника, то есть с обычных пассивных
# выводов. Без этих флагов ERC считает шины ничем не запитанными.
text("Питание берётся снаружи:", 25, 128, 1.8)
text("3.3 В с гребёнки, 12 В с клеммника", 25, 132, 1.8)
for i, kind in enumerate(("+3V3", "+12V", "GND")):
    pwr_flag(kind, (14 + 5 * i) * U, 55 * U)
